...
router.post('/getTermOfUseAccepted', requireRole([RoleLabel.SystemAdmin, RoleLabel.EventOrganizer, RoleLabel.EventSupporter]), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const termOfUseAccepted = await EventService.getTermOfUseAccepted(req.session.data.me);
    return res.status(200).send(createApiResponse<any>(null, termOfUseAccepted));
  } catch (err) {
    logger.error(err);
    return next(err);
  }
