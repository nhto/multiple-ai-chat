...
import { getCodeVerifier, getAuthorizationUrl, getLogoutUrl, getTokenSetAsync, getCodeChallenge } from '../../../utilities/polyusso';
...
router.get('/polyusso-callback', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const sub = "AD_" + String(req.session.data.idToken.sub);
...
router.get('/polyusso-without-account', async (req: Request, res: Response) => {
  const originUrl: string = !!req.query?.originUrl ? String(req.query.originUrl) : "/";
  ...
  try {
    req.session.data.me = await AuthnService.getNonRegisterUserMeSummary();
    req.session.netId = req.session.data.me?.netId;
